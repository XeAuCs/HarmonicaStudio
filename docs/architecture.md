...
谱面隐藏横向滚动条，保留浮点视图偏移和现有播放时钟。`NoteEditor` 区分音符编辑与谱面拖动：横移超过系统拖动阈值后通过 `scrubStarted` 请求控制层暂停，移动期间仅更新本地视图和进度展示，松手通过 `seekRequested` 一次性定位音频。暂停后定位保留暂停状态，不自动续播。忙碌或切换期间禁用谱面定位，替换文档或停止时清除拖动状态；视图不直接控制音频设备，不修改曲谱时间。隐藏滚动条的数值同步阻断信号，避免每帧触发额外滚动回调。

工程可选 `highlight` 字段表示单个心动片段起点，使用原谱秒数，必须为有限数字且处于 `[0, score_duration)`；无标记时省略字段，旧版格式 1 工程仍可读取。`set_highlight()` 统一校验、递增工程版本、使旧导出失效并请求自动暂存；播放中需先暂停，任务/切换期间禁止修改。改动不改变音符、移调或节奏；音符编辑使标记超出新曲尾时清除，重新提取的新工程不继承旧标记。

标记展示保留谱面上的原谱时间虚线，进度条用绘制的音符图形表示起点，不绘制文字标签。预览可用时进度条标记经 `TimeMap` 换算为实际音频秒数，否则采用原谱秒数；编辑使预览失效后同步恢复。名称调整不修改 `highlight`、`start_from_highlight` 等持久化字段，已有工程与设置兼容。

全局偏好 `start_from_highlight` 默认关闭，仅改变下一次播放起点，不修改工程版本。控制层在非暂停续播、非手动定位的试听开始时应用标记；导出前保留手动定位意图，停止及自然结束后恢复标记优先。游戏演奏和 F6 准备入口统一将标记经已有 `TimeMap` 换算为完整按键时间表上的起点。

`ScriptPlayer` 的可选 `start_seconds` 转为整数 `start_ms`，通过第四个 AHK 参数传入准备好的演奏器，或作为原命令协议的可选字段传入；没有偏移时保留原参数和命令格式。AHK 在本地内存中派生起点之后的事件，恢复仍在发声的按键和修饰键，保留后续相对时间；包含 100 毫秒准备段，修饰键提前 25 毫秒。状态进度仍报告完整时间表坐标，手机显示不因跳转而归零。完整脚本、WAV、MIDI 和工程音符不被截短；旧脚本遇到非零起点会明确要求重新导出。`--validate` / `--dump-events` 可带毫秒起点，在初始化定时器和热键前验证同一派生流程，不发送按键。
...
- `saved_revision` 表示打开时的版本或保存成功回执对应的快照版本。自动保存写入当前 `project_path` 时也推进；单独恢复暂存和导出不推进。保存期间继续编辑仍保持未保存状态。手动另存副本后，自动保存仍写入原关联工程，不把副本标记为已同步。
...
歌曲自动工程由 `song_projects.py` 定位：规范化源文件完整路径和 SHA-256 内容指纹共同派生数据目录 `song-projects/<key>.hstudio`，不修改曲库目录或 `catalog.json`。LOAD 工作线程读取源指纹及已有工程，仍准备原 MIDI 声部以支持主动重新提取；收取时经过原有取消/版本检查，有工程则安装原音符与标记，并按请求进入 EXPORT/试听，无工程则进入原转换流程。源内容改变或移动重命名产生新关联，旧工程保留。自动工程损坏或不可读时报告失败，不静默重新提取覆盖。

`AppState.auto_project_path` 保存当前自动工程目的地；直接打开工程时使用该工程路径。标记设置/清除立即在控制层提交自动保存，桌面音符编辑复用既有防抖暂存入口。AUTO、切换保护和 CLOSE 均通过现有串行 `SaveCoordinator` 写入恢复暂存和关联工程，每个文件沿用校验、临时文件及原子替换，不是跨文件事务；任一失败都不得确认整个请求成功或执行后续切换/关闭。保存快照固定目的地及工程版本，旧工程回执不能更新新工程状态。手动另存用于副本，不改变自动关联目标。
